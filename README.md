...
A single-file Python CLI that shows your Z.AI GLM Coding Plan usage: quota limits, token consumption, MCP tool calls, and a 24h trend chart, printed directly to the console. No dependencies, no config files, no emoji, just run it.
...
- **Quota Limits**: 5-hour token window, weekly token window, and monthly MCP quota with progress bars and reset countdowns
...
- **API key**: export it as an environment variable. The raw token is expected, with no `Bearer` prefix:
...
```bash
git clone https://github.com/your-username/glm-quota.git
cd glm-quota
python glm-quota.py
```

There is nothing to install. Run it with any Python 3.7+ interpreter.
...
If this tool has helped you, support is welcome:
